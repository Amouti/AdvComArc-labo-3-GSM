...
# source of this implementation https://github.com/dixitaayush8/A5-1/tree/master

def bytes_to_binary(data: bytes) -> str:
    return ''.join(f'{byte:08b}' for byte in data)

def binary_to_bytes(bit_string: str) -> bytes:
...
def A5_enc(pt, key):
	k_bits = bytes_to_binary(key)
	pt_bits = bytes_to_binary(pt)
	arr_bits = [int(c) for c in pt_bits]

	set_key(k_bits)

	keystream = get_keystream(len(pt_bits))
	i = 0
	s = ""
	while(i < len(pt_bits)):
		s = s + str(arr_bits[i] ^ keystream[i])
		i = i + 1
	return binary_to_bytes(s)

def A5_dec(pt, key):
	return A5_enc(pt, key)


test = b"\00\01"
print(bytes_to_binary(test))